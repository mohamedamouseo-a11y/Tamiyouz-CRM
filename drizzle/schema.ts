...
export const inAppNotifications = mysqlTable("in_app_notifications", {
	id: int().autoincrement().notNull(),
	userId: int().notNull(),
	type: mysqlEnum(['meeting_reminder','lead_assigned','sla_breach','lead_transfer','mention','system','stage_change','deal_won','deal_lost','activity_logged','lead_quality_change','duplicate_lead','bulk_import','data_edit','data_delete','data_restore','data_undo','new_lead','lead_distribution','campaign_alert','deal_update','contract_renewal','reminder','follow_up_reminder','internal_note']).default('system').notNull(),
...
// ─── TAM Meeting Briefs ─────────────────────────────────────────────────────
export const leadMeetingBriefs = mysqlTable("lead_meeting_briefs", {
id: int().autoincrement().notNull(),
leadId: int().notNull(),
tamUserId: int().notNull(),
salesUserId: int(),
meetingAt: timestamp({ mode: 'string' }).notNull(),
source: mysqlEnum('lead_meeting_brief_source', ['activity','calendar','manual']).default('manual').notNull(),
calendarEventId: varchar({ length: 255 }),
status: mysqlEnum('lead_meeting_brief_status', ['Draft','ReadyForMeeting','Completed']).default('Draft').notNull(),
talkingPoints: text(),
objectives: text(),
clientNeedsToHear: text(),
redFlags: text(),
prepareReminderId: int(),
salesReminderId: int(),
...
(table) => [
index("idx_meetingbrief_leadId").on(table.leadId),
index("idx_meetingbrief_tamUserId").on(table.tamUserId),
index("idx_meetingbrief_salesUserId").on(table.salesUserId),
index("idx_meetingbrief_status").on(table.status),
index("idx_meetingbrief_meetingAt").on(table.meetingAt),
primaryKey({ columns: [table.id], name: "lead_meeting_briefs_id"}),
]);
export type LeadMeetingBrief = typeof leadMeetingBriefs.$inferSelect;
export type InsertLeadMeetingBrief = typeof leadMeetingBriefs.$inferInsert;
...
export const landingPageIntegrations = mysqlTable("landing_page_integrations", {
  id: int().autoincrement().notNull(),
  name: varchar({ length: 255 }).notNull(),
  slug: varchar({ length: 255 }).notNull(),
  type: mysqlEnum(["landing_page"]).default("landing_page").notNull(),
  status: mysqlEnum(["Draft", "Active", "Disabled"]).default("Draft").notNull(),
  isEnabled: tinyint().default(0).notNull(),
  allowedDomains: json(),
  endpointPath: varchar({ length: 255 }).notNull(),
  sourceName: varchar({ length: 255 }),
  defaultPageTitle: varchar({ length: 255 }),
  defaultStage: varchar({ length: 100 }).default("New").notNull(),
  assignmentRule: mysqlEnum(["round_robin", "fixed_owner", "by_campaign"]).default("round_robin").notNull(),
  fixedOwnerId: int(),
  dedupRule: mysqlEnum(["phone", "external_id", "phone_and_source"]).default("phone").notNull(),
  attributionMode: mysqlEnum(["first_touch", "last_touch", "both"]).default("both").notNull(),
  fieldMapping: json(),
  securityConfig: json(),
  ...
  primaryKey({ columns: [table.id], name: "landing_page_integrations_id" }),
  index("idx_landing_page_integrations_slug").on(table.slug),
  index("idx_landing_page_integrations_deletedAt").on(table.deletedAt),
]);

export const landingPageSubmissionLogs = mysqlTable("landing_page_submission_logs", {
  id: int().autoincrement().notNull(),
  integrationId: int().notNull(),
  status: mysqlEnum(["received", "validated", "blocked", "pushed_to_crm", "duplicate", "failed"]).default("received").notNull(),
  ipAddress: varchar({ length: 128 }),
  userAgent: text(),
  origin: text(),
  payloadJson: json(),
  trackingJson: json(),
  ...
  primaryKey({ columns: [table.id], name: "landing_page_submission_logs_id" }),
  index("idx_lp_submission_integrationId").on(table.integrationId),
  index("idx_lp_submission_status").on(table.status),
]);

export type LandingPageIntegration = typeof landingPageIntegrations.$inferSelect;
export type InsertLandingPageIntegration = typeof landingPageIntegrations.$inferInsert;
export type LandingPageSubmissionLog = typeof landingPageSubmissionLogs.$inferSelect;
export type InsertLandingPageSubmissionLog = typeof landingPageSubmissionLogs.$inferInsert;
