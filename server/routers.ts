...
import { rakanExecutiveRouter } from "./rakanExecutiveRouter";
    ...
    list: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (ctx.user.role === "SalesAgent") {
          // SalesAgent sees leads they own OR are assigned to (via lead_assignments)
    ...
    byId: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (ctx.user.role === "SalesAgent" && lead.ownerId !== ctx.user.id) {
          // Check if user has access via lead_assignments
          const access = await checkLeadAccess(input.id, ctx.user.id);
          if (!access.hasAccess) {
    ...
    update: notMediaBuyerProcedure
      .input(
        z.object({
          ...
          fitStatus: z.enum(["Fit", "Not Fit", "Pending"]).optional(),
          campaignName: z.string().optional(),
          adCreative: z.string().optional(),
  ...
  deals: router({
    ...
    create: notMediaBuyerProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const existingDeal = await getDealByLead(input.leadId);
        if (existingDeal && existingDeal.deletedAt === null) {
          const dealAge = Date.now() - new Date(existingDeal.createdAt).getTime();
          if (dealAge < 60000 && existingDeal.valueSar === (input.valueSar ?? null)) {
            return { id: existingDeal.id };
          }
        }
    ...
    addCollaborator: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          const roleLabelAr = roleLabelsAr[input.role] ?? input.role;
          const leadDisplay = lead.name || lead.phone || `#${lead.id}`;
          await createInAppNotification({
            userId: input.userId,
            type: "lead_assigned" as any,
            title: `You've been assigned as ${roleLabel} on lead: ${leadDisplay}`,
            titleAr: `تم تعيينك ك${roleLabelAr} على الليد: ${leadDisplay}`,
            body: input.reason ? `Reason: ${input.reason}` : null,
            bodyAr: input.reason ? `السبب: ${input.reason}` : null,
            isRead: false,
            link: `/leads/${input.leadId}`,
            metadata: { leadId: input.leadId, trigger: "lead_assigned", assignmentRole: input.role },
          } as any);
        } catch (e) {
          console.error("[Notification] Failed to notify assignment:", e);
        }
  ...
  accountManagement: router({
    ...
    createClient: accountManagerProcedure
      .input(z.object({
        ...
        planStatus: z.enum(["Active", "Paused", "Cancelled", "Pending", "Hold"]).default("Active"),
    ...
    updateClient: accountManagerProcedure
      .input(z.object({
        ...
        businessProfile: z.string().optional(),
        group: z.string().optional(),
        planStatus: z.enum(["Active", "Paused", "Cancelled", "Pending", "Hold"]).optional(),
    ...
    createContract: accountManagerProcedure
      .input(z.object({
        ...
        packageIds: z.array(z.number()).optional().nullable(),
      ...
      .mutation(async ({ input }) => {
        ...
        // Handle packageIds: store as JSON array, also set packageId to first item for backward compat
        if (input.packageIds && input.packageIds.length > 0) {
          contractData.packageIds = input.packageIds;
          contractData.packageId = input.packageIds[0];
        } else {
          contractData.packageIds = null;
        }
    ...
    updateContract: accountManagerProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (data.packageIds !== undefined) {
          contractData.packageIds = data.packageIds && data.packageIds.length > 0 ? data.packageIds : null;
          contractData.packageId = data.packageIds && data.packageIds.length > 0 ? data.packageIds[0] : null;
        }
...
// ─── Meta Ads Integration ──────────────────────────────────────────────────
  ...
  rakan: rakanRouter,
  rakanExecutive: rakanExecutiveRouter,
  tamara: tamaraRouter,
  paymob: paymobRouter,
