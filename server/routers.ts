  ...
  supportCenter: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Deduplication: reject if same user submitted same subject within last 30 seconds
        const thirtySecondsAgo = new Date(Date.now() - 30_000);
        const recentDuplicates = await db.select({ id: supportRequests.id })
          .from(supportRequests)
          .where(and(
            eq(supportRequests.createdBy, ctx.user.id),
            eq(supportRequests.subject, input.subject),
            gte(supportRequests.createdAt, thirtySecondsAgo),
          ))
          .limit(1);
        if (recentDuplicates.length > 0) {
          throw new TRPCError({ code: "CONFLICT", message: "A similar request was just submitted. Please wait before submitting again." });
        }
