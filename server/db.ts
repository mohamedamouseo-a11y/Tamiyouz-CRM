...
const DEFAULT_ONBOARDING_PHASES = [
  {
    phase: 2,
    phaseLabel: "Phase 2: Account Manager Preparation",
    items: [
      "Review all client information",
      "Review contract",
      "Identify missing information",
      "Prepare questions for client",
      "Prepare initial plan concept",
    ],
  },
  {
    phase: 3,
    phaseLabel: "Phase 3: First Client Contact",
    items: [
      "Contact client within 24 hours",
      "Introduce yourself as point of contact",
      "Schedule onboarding meeting",
    ],
  },
  {
    phase: 4,
    phaseLabel: "Phase 4: Onboarding Meeting",
    items: [
      "Introduce team and roles",
      "Understand business more deeply",
  ...
  {
    phase: 5,
    phaseLabel: "Phase 5: Access & Setup",
    items: [
      "Receive social media access",
      "Receive website access",
      "Receive ads access if applicable",
      "Add client to communication channels",
      "Record all client info in CRM",
    ],
  },
  {
    phase: 6,
    phaseLabel: "Phase 6: Internal Team Activation",
    items: [
      "Send clear internal brief to each department",
      "Define objectives for each department",
      "Define deadlines for monthly reports",
      "Hold internal team meeting if needed",
    ],
  },
];
...
export interface CrmCampaignStats {
  ...
  fitCount: number;
  notFitCount: number;
  pendingFitCount: number;
  leadCount: number;      // classification: Lead
  prospectCount: number;  // classification: Prospect
  opportunityCount: number; // classification: Opportunity
  avgScore: number;
  totalDeals: number;
  wonDeals: number;
  lostDeals: number;
  pendingDeals: number;
  wonRevenue: number;     // in base currency (SAR)
  totalRevenue: number;   // in base currency (SAR)
}

export async function getCrmStatsByCampaign(dateFrom?: string, dateTo?: string): Promise<CrmCampaignStats[]> {
  const db = await getDb();
  if (!db) return [];

  // 1. Get all leads grouped by campaign
  const hasDateFilter = dateFrom && dateTo;
  const leadRows = hasDateFilter
    ? await db.execute(sql`
        SELECT l.id, l.campaignName, l.leadQuality, l.fit_status as fitStatus, l.stage
        FROM leads l
        WHERE l.deletedAt IS NULL AND l.createdAt >= ${dateFrom} AND l.createdAt <= ${dateTo}
        ORDER BY l.campaignName
      `)
    : await db.execute(sql`
        SELECT l.id, l.campaignName, l.leadQuality, l.fit_status as fitStatus, l.stage
        FROM leads l
  ...
  const allLeads: any[] = (leadRows as any)[0] ?? [];
  if (allLeads.length === 0) return [];

  // 2. Get activity counts per lead for scoring
  ...
  if (leadIds.length > 0) {
    ...
    for (let i = 0; i < leadIds.length; i += 500) {
      ...
      const actRows = await db.execute(sql`
        ...
        FROM activities
        WHERE leadId IN (${sql.join(chunk.map(id => sql`${id}`), sql`, `)})
          AND deletedAt IS NULL
        GROUP BY leadId
      `);
      for (const r of ((actRows as any)[0] ?? [])) {
        activityCounts[r.leadId] = Number(r.cnt ?? 0);
      }
    }
  }

  // 3. Get all deals for these leads
  let dealsByLeadId: Record<number, { status: string; valueBase: number }[]> = {};
  if (leadIds.length > 0) {
    for (let i = 0; i < leadIds.length; i += 500) {
      const chunk = leadIds.slice(i, i + 500);
      const dealRows = await db.execute(sql`
        SELECT leadId, status, COALESCE(valueBase, valueSar, '0') as valueBase
        FROM deals
        WHERE leadId IN (${sql.join(chunk.map(id => sql`${id}`), sql`, `)})
          AND deletedAt IS NULL
      `);
      for (const r of ((dealRows as any)[0] ?? [])) {
        if (!dealsByLeadId[r.leadId]) dealsByLeadId[r.leadId] = [];
        dealsByLeadId[r.leadId].push({
          status: r.status,
          valueBase: parseFloat(r.valueBase || "0"),
        });
      }
    }
  }

  // 4. Lead classification logic (same as LeadsList.tsx)
  function classifyLead(stage: string, quality: string, fitStatus: string): "Lead" | "Prospect" | "Opportunity" {
  ...
  // 5. Lead score calculation (same as LeadProfile.tsx)
  const qualityScoreMap: Record<string, number> = { Hot: 30, Warm: 20, Cold: 10, Bad: 0, Unknown: 5 };
  const fitScoreMap: Record<string, number> = { Fit: 15, "Not Fit": -20, Pending: 0 };

  function calcLeadScore(activitiesCount: number, quality: string, hasDeal: boolean, fitStatus: string): number {
    const activityScore = Math.min(activitiesCount * 10, 50);
    const qualityScore = qualityScoreMap[quality] ?? 5;
    const dealScore = hasDeal ? 20 : 0;
    const fitScore = fitScoreMap[fitStatus] ?? 0;
    return Math.max(0, Math.min(100, activityScore + qualityScore + dealScore + fitScore));
  }

  // 6. Aggregate per campaign
  const campaignMap: Record<string, CrmCampaignStats> = {};

  for (const lead of allLeads) {
    const campName = lead.campaignName || "Unassigned";
    if (!campaignMap[campName]) {
      campaignMap[campName] = {
        campaignName: campName,
        totalLeads: 0, hot: 0, warm: 0, cold: 0, bad: 0, unknown: 0,
        fitCount: 0, notFitCount: 0, pendingFitCount: 0,
        leadCount: 0, prospectCount: 0, opportunityCount: 0,
        avgScore: 0, totalDeals: 0, wonDeals: 0, lostDeals: 0, pendingDeals: 0,
        wonRevenue: 0, totalRevenue: 0,
      };
    }
    const stats = campaignMap[campName];
    stats.totalLeads++;
    ...
    switch (lead.leadQuality) {
      ...
      case "Bad": stats.bad++; break;
      default: stats.unknown++; break;
    }

    // Fit status
    switch (lead.fitStatus) {
      case "Fit": stats.fitCount++; break;
      case "Not Fit": stats.notFitCount++; break;
      default: stats.pendingFitCount++; break;
    }

    // Classification
    const classification = classifyLead(lead.stage || "New", lead.leadQuality || "Unknown", lead.fitStatus || "Pending");
    switch (classification) {
      case "Lead": stats.leadCount++; break;
      case "Prospect": stats.prospectCount++; break;
      case "Opportunity": stats.opportunityCount++; break;
    }

    // Lead score
    const leadDeals = dealsByLeadId[lead.id] || [];
    const hasDeal = leadDeals.length > 0;
    const score = calcLeadScore(activityCounts[lead.id] || 0, lead.leadQuality || "Unknown", hasDeal, lead.fitStatus || "Pending");
    stats.avgScore += score;

    // Deals
    for (const deal of leadDeals) {
      stats.totalDeals++;
      stats.totalRevenue += deal.valueBase;
      switch (deal.status) {
        case "Won": stats.wonDeals++; stats.wonRevenue += deal.valueBase; break;
        case "Lost": stats.lostDeals++; break;
        default: stats.pendingDeals++; break;
      }
    }
  }
  ...
  for (const stats of result) {
    stats.avgScore = stats.totalLeads > 0 ? Math.round(stats.avgScore / stats.totalLeads) : 0;
  }
