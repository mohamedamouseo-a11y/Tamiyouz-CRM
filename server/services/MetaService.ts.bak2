...
export async function fetchAllCampaignInsights(accountDbId: number, datePreset: string = "last_30d") {
  ...
  for (const c of campaigns) {
    ...
    if (insights) {
      ...
      const leadAction = insights.actions?.find((a: any) => a.action_type === "lead" || a.action_type === "onsite_conversion.lead_grouped");
      ...
      const cplAction = insights.cost_per_action_type?.find((a: any) => a.action_type === "lead" || a.action_type === "onsite_conversion.lead_grouped");
