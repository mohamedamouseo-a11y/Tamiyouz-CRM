...
export async function rakanChat(
  ...
  userName: string,
  userMessage: string,
  ttsVoicePreference?: string // user preference: 'ar_formal' | 'ar_egyptian' | 'ar_gulf' | 'en' | 'none'
): Promise<{ reply: string; audioBase64: string | null; mode: "smart" | "normal" }> {
  // 1. Get settings
  ...
  // 2. Build DB context (needed for all modes)
  const dbContext = await buildDbContext(userId, role, userMessage);

  // 3. Build system prompt (shared between Gemini and fallback LLM)
...
- اتكلم بالمصري أو بالعربي الطبيعي`;
  ...
  // 5. Try LLMs in order: Gemini → Fallback LLM → Static
  let reply: string;
  ...
  if (geminiKey) {
    try {
      ...
      // ─── Tier 2: Fallback LLM (OpenAI-compatible) ───
      if (fallbackApiKey) {
        try {
        ...
        // No fallback LLM configured, go to static
        reply = generateStaticFallbackReply(rakanName, userName, role, userMessage, dbContext);
        mode = "normal";
    ...
    // No Gemini key, try fallback LLM directly
    try {
      reply = await callOpenAICompatible(
        fallbackApiKey,
        fallbackBaseUrl,
  ...
  if (voicePref !== "none" && ttsKey) {
    ...
    // Limit TTS to 500 chars to avoid large responses
