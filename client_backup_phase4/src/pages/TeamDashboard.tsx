      ...
      <div className="p-6 space-y-6 fade-in" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <Link key={i} href={(card as any).href}><Card className="cursor-pointer hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white" style={{ background: card.color }}>
                    {card.icon}
                  </div>
                </div>
              ...
              {isLoading ? (
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={stats?.leadsByAgent ?? []} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                    ...
                    />
                    <Bar dataKey="count" fill={tokens.primaryColor} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </CardContent>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={stats?.dealsByAgent ?? []} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                    ...
                    <Tooltip
                      contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8 }}
                    />
                    <Bar dataKey="wonDeals" fill={tokens.successColor} radius={[4, 4, 0, 0]} name={t("wonDeals")} />
                    <Bar dataKey="revenue" fill={tokens.accentColor} radius={[4, 4, 0, 0]} name={t("totalRevenue")} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </CardContent>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    <tr>
                      <td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">{t("noData")}</td>
                    </tr>
                  ) : (
