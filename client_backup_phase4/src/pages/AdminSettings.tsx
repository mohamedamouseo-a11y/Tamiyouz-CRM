...
import { Bell, Copy, Edit, Eye, EyeOff, FileSpreadsheet, GripVertical, Mail, Palette, Plus, RefreshCw, Save, Send, Settings, Shield, Sliders, Trash2, Users, Check, Archive } from "lucide-react";
...
const ROLES = ["Admin", "SalesManager", "SalesAgent", "MediaBuyer"];
...
export default function AdminSettings() {
  ...
  return (
    <CRMLayout>
      <div className="p-6 space-y-6 fade-in" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <Tabs defaultValue="users">
          <TabsList className="flex-wrap h-auto gap-1">
            <TabsTrigger value="users" className="gap-1.5"><Users size={14} />{t("users")}</TabsTrigger>
            <TabsTrigger value="pipeline" className="gap-1.5"><Sliders size={14} />{t("pipelineStages")}</TabsTrigger>
            <TabsTrigger value="campaigns" className="gap-1.5"><Settings size={14} />{t("campaigns")}</TabsTrigger>
            <TabsTrigger value="fields" className="gap-1.5"><Shield size={14} />{t("customFields")}</TabsTrigger>
            <TabsTrigger value="theme" className="gap-1.5"><Palette size={14} />{t("themeSettings")}</TabsTrigger>
            <TabsTrigger value="sla" className="gap-1.5"><Settings size={14} />{t("slaSettings")}</TabsTrigger>
            <TabsTrigger value="notifications" className="gap-1.5"><Bell size={14} />{isRTL ? "الإشعارات" : "Notifications"}</TabsTrigger>
            <TabsTrigger value="leadSources" className="gap-1.5"><FileSpreadsheet size={14} />{isRTL ? "مصادر العملاء" : "Lead Sources"}</TabsTrigger>
            <TabsTrigger value="backup" className="gap-1.5"><Archive size={14} />{isRTL ? "النسخ الاحتياطية" : "Backup"}</TabsTrigger>
          </TabsList>
                  ...
                  <tbody>
                    {(users ?? []).map((u) => (
                      <tr key={u.id} className="border-b border-border hover:bg-muted/30">
                        ...
                        <td className="px-4 py-3">
                          <span className={`text-xs font-medium ${(u as any).isActive !== false ? "text-green-600" : "text-red-500"}`}>
                            {(u as any).isActive !== false ? t("active") : t("inactive")}
                          </span>
                        </td>
                        <td className="px-4 py-3">
          ...
          {/* ── Lead Sources Tab ── */}
          <TabsContent value="leadSources" className="mt-4">
            <LeadSourcesTab />
          </TabsContent>
      ...
      <Dialog open={showAddUser} onOpenChange={(v) => { setShowAddUser(v); if (!v) { setEditingUser(null); setShowPassword(false); } }}>
