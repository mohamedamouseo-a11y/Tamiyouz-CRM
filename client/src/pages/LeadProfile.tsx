                ...
                <Tabs defaultValue="basic" className="w-full">
                  <CardHeader className="pb-2 pt-3">
                    <TabsList className="w-full">
                      <TabsTrigger value="basic" className="flex-1 text-xs">{t("basicInfo")}</TabsTrigger>
                      <TabsTrigger value="form" className="flex-1 text-xs">{t("formDataTab" as any)}</TabsTrigger>
                      <TabsTrigger value="sales" className="flex-1 text-xs">{t("salesInfo")}</TabsTrigger>
                    </TabsList>
                  </CardHeader>
                        ...
                        return (
                          <div key={stage} className="flex items-center">
                            <div className="flex flex-col items-center gap-1 min-w-[90px]">
                              <div
                                className={`flex h-8 w-8 items-center justify-center rounded-full border-[3px] bg-background text-xs font-bold transition-all ${isCurrent ? 'ring-2 ring-offset-1' : ''}`}
                                style={{ borderColor: color, color, ...(isCurrent ? { ringColor: `${color}40` } : {}) }}
                              >
                                {isCompleted ? <CheckCircle size={14} style={{ color }} /> : index + 1}
                              </div>
                              <p className="text-[11px] font-medium text-foreground text-center whitespace-normal leading-tight max-w-[85px]">
                            ...
                            {index !== stageList.length - 1 && (
                              <div
                                className="h-[3px] w-6 rounded-full mx-0.5 shrink-0"
                    ...
                    {composerMode === "note" ? (
                      <div className="space-y-2">
                        ...
                        <Textarea
                          ...
                          placeholder={isRTL ? "اكتب ملاحظة سريعة أو التحيث الأهم عن العميل..." : "Write a quick note or the most important update about this lead..."}
