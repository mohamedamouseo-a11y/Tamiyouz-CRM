...
export default function TaskSlaDashboard() {
  ...
  const { data: salesAgents } = trpc.users.list.useQuery(undefined, { enabled: true });
  const filteredAgents = (salesAgents ?? []).filter((u: any) => u.role === 'SalesAgent');
  ...
  const selectedAgentName = selectedAgentId
    ? filteredAgents.find((a: any) => a.id === selectedAgentId)?.name ?? ""
    : (isRTL ? "الكل" : "All Agents");
    ...
    <CRMLayout>
      <div className="p-6 space-y-6 fade-in" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          {/* Global Filters: Date Range + Agent Dropdown */}
          <div className="flex items-center gap-3 flex-wrap">
            {isManager && (
              <div className="flex items-center gap-2">
                <label className="text-xs font-medium text-muted-foreground whitespace-nowrap">
                  <Users size={14} className="inline-block mr-1 mb-0.5" />
                  {isRTL ? "وكيل المبيعات" : "Sales Agent"}
                </label>
                <select
                  className="text-sm border border-border rounded-lg px-3 py-2 bg-background text-foreground shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary min-w-[160px]"
                  value={selectedAgentId ?? ""}
                  onChange={(e) => setSelectedAgentId(e.target.value ? Number(e.target.value) : null)}
                >
                  <option value="">{isRTL ? "الكل" : "All Sales Agents"}</option>
