...
function ReportDownloadButton({ report, isRTL }: { report: ReportPayload; isRTL: boolean }) {
  ...
  const handleDownload = () => {
    ...
    try {
      if (report.url) {
        // Server-side file: open in new tab
        window.open(report.url, "_blank");
      } else if (report.base64) {
        downloadBase64File(report.base64, report.fileName, report.mimeType);
      }
      toast.success(isRTL ? "جاري تحميل التقرير..." : "Downloading report...");
    } catch (err) {
  ...
  return (
    <button
      ...
      className={cn(
        "mt-2 inline-flex items-center gap-2 rounded-xl border border-emerald-300 dark:border-emerald-700",
        "bg-emerald-50 dark:bg-emerald-900/30 px-3.5 py-2 text-sm font-medium",
    ...
    >
      ...
      <span>{isRTL ? "تحميل التقرير" : "Download Report"}</span>
      <span className="text-xs opacity-60">{report.fileName}</span>
    </button>
  );
}
...
export default function RakanWidget() {
  ...
  // ── Chat mutation (now handles both text and report responses) ─────────────
  const chatMutation = trpc.rakan.chat.useMutation({
    onSuccess: (data: any) => {
      const isReport = data.kind === "report" && data.report;
      ...
      const assistantMsg: ChatMessage = {
        ...
        mode: data.mode,
        kind: data.kind || "text",
        report: isReport ? data.report : undefined,
        timestamp: new Date(),
      };
      setMessages(prev => [...prev, assistantMsg]);
      setIsSending(false);

      // Auto-play audio if enabled (only for text responses)
      if (ttsEnabled && !isReport) {
        const replyText = data.answer || data.reply || "";
        if (data.audioBase64) {
          playAudio(assistantMsg.id, data.audioBase64);
        } else {
          speakWithBrowser(replyText, assistantMsg.id);
        }
      }
              ...
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full gap-3 text-center px-4">
                  ...
                  <p className="text-muted-foreground text-xs leading-relaxed">
                    {isRTL
                      ? "اسألني عن أي شيء في النظام - الليدز، الصفقات، الأداء، العملاء... أو اطلب تقرير Excel 📊"
                      : "Ask me anything about the system - leads, deals, performance, clients... or request an Excel report 📊"}
                  </p>
                  <div className="flex flex-wrap gap-1.5 justify-center mt-1">
                    {(isRTL
                      ? ["كم ليد عندي اليوم؟", "من أفضل سيلز؟", "عايز تقرير أداء السيلز", "هات شيت بالتجديدات"]
                      : ["How many leads today?", "Show my performance", "Sales performance report", "Contracts report"]
                    ).map((q) => (
                      <button
                        key={q}
              ...
              {messages.map((msg) => (
                ...
                >
                  ...
                  <div
                    className={cn(
                      ...
                      msg.role === "user"
                        ...
                        : msg.kind === "report"
                          ? "bg-emerald-50 dark:bg-emerald-900/20 text-foreground rounded-tl-sm border border-emerald-200 dark:border-emerald-800"
                  ...
                  >
                    ...
                    {/* Report download button */}
                    {msg.report && (
                      <ReportDownloadButton report={msg.report} isRTL={isRTL} />
                    )}

                    {/* Mode indicator for assistant messages */}
                    {msg.role === "assistant" && msg.mode && !msg.report && (
                      <div className={cn(
                    ...
                    {/* Report badge */}
                    {msg.kind === "report" && (
                      <div className="mt-1 inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">
                        <FileSpreadsheet size={8} /> {isRTL ? "تقرير Excel" : "Excel Report"}
                      </div>
                    )}

                    {/* Audio button for assistant messages (not for reports) */}
            ...
            <div className="px-3 py-2.5 border-t border-border bg-background">
              <div className="flex gap-2 items-end">
                ...
                <Textarea
                  ...
                  placeholder={isRTL ? "اسأل راكان... أو اطلب تقرير 📊" : "Ask Rakan... or request a report 📊"}
                  className={cn(
              ...
              <p className="text-[10px] text-muted-foreground text-center mt-1.5">
                {isRTL ? "Enter للإرسال • Shift+Enter لسطر جديد • 🎤 للتحدث • 📊 للتقارير" : "Enter to send • Shift+Enter for new line • 🎤 to talk • 📊 for reports"}
              </p>
            </div>
          </div>
        )}
