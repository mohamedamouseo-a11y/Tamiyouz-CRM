...
function renderContent(text: string) {
  ...
  const urlRegex = /(https?:\/\/[^\s]+|\/downloads\/rakan\/[^\s]+)/g;
  ...
  const renderTextWithLinks = (line: string) => {
    ...
    return parts.map((part, idx) => {
      ...
      urlRegex.lastIndex = 0;
      return part.startsWith("**") && part.endsWith("**") ? <strong key={idx}>{part.slice(2, -2)}</strong> : part;
    });
  };
  ...
  return lines.map((line, i) => {
    ...
    if (line.startsWith("- ") || line.startsWith("• ")) {
      return <p key={i} className="flex gap-1"><span>•</span><span>{renderTextWithLinks(line.slice(2))}</span></p>;
    }
    if (line.trim() === "") return <br key={i} />;
    return <p key={i}>{renderTextWithLinks(line)}</p>;
  });
}
