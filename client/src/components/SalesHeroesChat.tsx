...
export default function SalesHeroesChat() {
  ...
  return (
    ...
    >
      {isOpen && (
        <Card
          className="mb-3 w-80 md:w-[360px] shadow-2xl border-border/60 flex flex-col overflow-hidden rounded-2xl"
          style={{
        ...
        >
          <CardHeader className="p-3 text-white flex flex-row items-center justify-between shrink-0" style={{ background: "linear-gradient(135deg, var(--primary) 0%, color-mix(in srgb, var(--primary) 75%, #7c3aed) 100%)" }}>
              ...
              <div className="min-w-0">
                ...
                {selectedUser && (
                  <div className="text-[11px] opacity-90 flex items-center gap-2">
                    ...
                    {selectedUser.role && (
                      <span className="px-1.5 py-0 rounded-full bg-white/20 text-[9px] font-medium">
                        {selectedUser.role === "Admin" ? (isRTL ? "مدير" : "Admin")
                          : selectedUser.role === "SalesManager" ? (isRTL ? "مدير مبيعات" : "Sales Mgr")
                          : selectedUser.role === "SalesAgent" ? (isRTL ? "مندوب" : "Agent")
                          : selectedUser.role === "AccountManager" ? (isRTL ? "مدير حسابات" : "Acc. Mgr")
                          : selectedUser.role}
                      </span>
                    )}
                  </div>
          ...
          {!isMinimized && (
            <CardContent className="p-0 flex-1 flex flex-col overflow-hidden bg-background min-h-0">
              {selectedUser ? (
                <>
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    {u.role && (
                                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-primary/10 text-primary font-medium whitespace-nowrap">
                                        {u.role === "Admin" ? (isRTL ? "مدير" : "Admin")
                                          : u.role === "SalesManager" ? (isRTL ? "مدير مبيعات" : "Sales Manager")
                                          : u.role === "SalesAgent" ? (isRTL ? "مندوب مبيعات" : "Sales Agent")
                                          : u.role === "AccountManager" ? (isRTL ? "مدير حسابات" : "Account Manager")
                                          : u.role}
                                      </span>
                                    )}
                                  </div>
      ...
      <button
        className="h-11 w-11 rounded-full shadow-[0_4px_16px_rgba(37,99,235,0.35)] hover:shadow-[0_6px_24px_rgba(37,99,235,0.5)] touch-none transition-all duration-300 hover:scale-110 active:scale-95 bg-gradient-to-br from-blue-500 to-blue-600 border-2 border-white/30 flex items-center justify-center relative cursor-pointer"
        style={{ outline: "none" }}
      ...
      >
        <img src={LOGO_URL} alt="Tamiyouz Chat" className="w-6 h-6 object-contain rounded-full" />
        {totalUnread > 0 && (
          <div className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 bg-red-500 text-white rounded-full text-[10px] font-bold flex items-center justify-center border border-white shadow-sm">
